...
struct Lattice {
    ...
    // Lattice(const Lattice& original, const vector<vector<double>>& deformationMatrix) {
    //     N = original.N;
    //     a = original.a;
    //     fccBasis = original.fccBasis;
    //     // neighbours = initNeighbours()

    //     for (const Atom& atom : original.atoms) {
    //         Vector newCoord = atom.coord.multiplyByMatrix(deformationMatrix);
    //         atoms.push_back(Atom(newCoord, atom.type));
    //     }

    //     for (const Vector& ngb : original.neighbours) {
    //         Vector newCoord = ngb.multiplyByMatrix(deformationMatrix);
    ...
    vector<Vector> initNeighbours(double cutoff, double delta = 0) { // предположим, что 1.7a не меняется при диформации решетки 
        vector<Vector> neighbours;
        ...
        for (nb.x = center.x - coordCutoff; nb.x <= center.x + cutoff; nb.x += 0.5) { 
            for (nb.y = center.y - coordCutoff; nb.y <= center.y + cutoff; nb.y += 0.5) {
                for (nb.z = center.z - coordCutoff; nb.z <= center.z + cutoff; nb.z += 0.5) {
                   // double r = nb.multiplyByVector(Vector(1+delta, 1+delta, 1+delta)).length()
    ...
    // int getNumAtomInBasis(const Vector &atom) const {
    //     atom.modf();
    //     int index = 0;
    //     for (const Vector& basisAtom : fccBasis) {
    //         if (atom.equals(basisAtom)) {
    ...
    // int getIndex(const Vector& atom) const {
...
struct Elasticity {
    double B(Energy energy, Lattice originalLattice, double delta) const {
        ...
        cout << secondDerivative << endl;
        return (2*secondDerivative)/(9*originalLattice.V);
    }
...
int main()
{
    ...
    cout << "Модуль упругости: " << elasticity.B(energy, lattice, 0.001) << endl;
